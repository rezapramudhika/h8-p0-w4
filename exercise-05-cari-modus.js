/*Problem
Diberikan sebuah function cariModus(arr) yang menerima sebuah array angka. 
Function akan me-return modus dari array atau deret angka tersebut. 
Modus adalah angka dari sebuah deret yang paling banyak atau paling sering muncul. 
Contoh, modus dari [10, 4, 5, 2, 4] adalah 4. Jika modus tidak ditemukan, 
function akan me-return -1. Apabila ditemukan lebih dari dua nilai modus, 
tampilkan nilai modus yang paling pertama muncul (dihitung dari kiri ke kanan). Dan apabila didalam 
modus hanya ada 1 nilai yang sama maka function akan me-return -1, Contohnya [1, 1, 1] adalah -1. */

function cariModus(arr) {
    var temp = [];
    for(var i=0; i<=arr.length-1; i++){
        temp[i] = 1;
        for (var j=i+1; j<=arr.length-1; j++){
            if(arr[j]===arr[i]){
                temp[i] += 1;
            }
        }
    }

    var checkTemp=0;
    for (var k=0; k<=temp.length-1; k++){
        if(temp[k]==1){
            checkTemp += 1;
        }
    }

    if (checkTemp==arr.length){
        return -1;
    }else if(temp[0]==arr.length){
        return -1;
    }else{
        var temp2=0;
        var temp3=0;
        for(var x=temp.length-1; x>=0; x--){
            if(temp[x]==temp2){
                temp2 = temp[x];
                temp3 = arr[x];
            }else if(temp[x]>temp2){
                    temp2 = temp[x];
                    temp3 = arr[x];
            }
    
        }
        return(temp3)
    }

  }
  
  // TEST CASES
  console.log(cariModus([10, 4, 5, 2, 4])); // 4
  console.log(cariModus([5, 10, 10, 6, 5])); // 5
  console.log(cariModus([10, 3, 1, 2, 5])); // -1
  console.log(cariModus([1, 2, 3, 3, 4, 5])); // 3
  console.log(cariModus([7, 7, 7, 7, 7])); // -1